...
![App-props-route](./images/11-App-props-route.png)
...
![代理](./images/12-proxy.png)

+ 我们采用 express-http-proxy 这个库来做转发，但是转发之前我们要考虑一下，哪些需要转发？哪些不需要转发？肯定不是所有的都转发，我们要知道第三方服务是做数据提供的，不是做页面渲染和资源提供的，所以只需要转发 api 数据接口就行，其他的不需要也不能转发，所以，我们统一把以 api 开头的接口，转发给 Java 服务器
...
+ 我们知道，我们现在的浏览器向服务器发送请求的时候，是有 cookie 信息的，但是服务端再向 Java 服务器请求的时候，是没有 cookie 信息的，但是我们经常需要做登录校验的判断，所以我们还需要把 cookie 转发给 Java 服务器
